// 
// REQUIREMENTS.JS — AGENTIC ADVISOR
// -----------------------------------------------------------------------------
// This file defines all degree requirement structures for supported majors,
// the breadth course matcher logic, and pathway data.
//
// HOW TO ADD A NEW MAJOR:
//   Add a new key under REQUIREMENTS:
//   REQUIREMENTS["Major Name"] = { "Concentration Name": { core, concentration, distribution, pathway } }
//
// STRUCTURE OF EACH REQUIREMENT ENTRY:
//   { requirement: "Display name", code: "SINGLE_CODE", options: [] }
//   - If code is set and options is empty → single fixed course, no dropdown
//   - If code is empty and options has items → dropdown selector shown to user
//   - If neither → informational note shown using req.note
// 


// 
// REQUIREMENTS — HARDCODED DEGREE STRUCTURES BY MAJOR + CONCENTRATION
// Server cross-references AI-extracted completions against these to determine
// what's remaining. Much more reliable than asking the AI to figure it out.
// 
const REQUIREMENTS = {
  "Computer Science": {
    "Software Development": {

      // CORE MAJOR REQUIREMENTS — fixed single courses, no alternatives
      core: [
      ...
      // CONCENTRATION REQUIREMENTS — choose-one dropdowns
      // Note: courses chosen here should be cross-checked so the same course
      // can't be selected in multiple buckets simultaneously (handled in index.html)
      concentration: [
        {
        ...
        {
          // Third elective shares many options with System and Language electives.
          // If a course is picked above, it gets grayed out here in the UI.
      ...
      // DISTRIBUTION / BREADTH REQUIREMENTS — Marist Core requirements
      // Fixed single-course slots use code field only.
      // Open breadth slots (History, Fine Arts, etc.) use options arrays.
      // Breadth matching is also handled by BREADTH_MATCHERS below for server-side detection.
      distribution: [
        ...
        {
          // History breadth: any 100-200 level HIST course (except HIST 209L)
          // or 300-level with 6 prior history credits
        ...
        {
          // Math breadth: most 100-level MATH (110+) or any 200-level MATH
          // MATH 108 and MATH 180N do NOT count
        ...
        {
          // Ethics breadth: includes any REST (Religious Studies) course
          // Applied ethics options from CS, Media, Criminal Justice, Business
      ...
      // PATHWAY REQUIREMENTS — shown as informational rows with credit counters
      // These are tracked via distribution_credits_earned and liberal_arts_credits_earned
      // extracted from the PDF, not by course code matching
      pathway: [
        {
...
// 
// BREADTH MATCHERS — DISCIPLINE-BASED COURSE CODE MATCHING
// Used server-side to determine if a breadth requirement has been fulfilled
// without needing an exact course code match (handles transfer/AP variants).
//
// Each matcher can have:
//   codes    — exact course codes that fulfill this breadth area
//   prefixes — any code starting with this prefix qualifies
//   excludes — codes that do NOT qualify despite matching prefix
// 
const BREADTH_MATCHERS = {
  history: {
    prefixes: ["HIST"],
    excludes: ["HIST 209L"], // HIST 209L explicitly does not count
  },
  math: {
    codes: ["MATH 110L", "MATH 111L", "MATH 130L", "MATH 241L"],
    prefixes: ["MATH 2"], // any 200-level MATH qualifies
  },
  naturalScience: {
  ...
  socialScience: {
    ...
    prefixes: ["POSC 2", "SOC 2"] // any 200-level POSC or SOC qualifies
  },
  fineArts: {
    ...
    prefixes: ["MUS 3"] // any 300-level MUS course qualifies
  },
  ethics: {
    codes: ["PHIL 200L", "PHIL 247L", "PHIL 301L", "PHIL 302L", "PHIL 346L", "PHIL 348L",
      "CMPT 305L", "MDIA 316L", "CRJU 310L", "BUS 312L", "BUS 319N"],
    prefixes: ["REST"] // any REST (Religious Studies) course qualifies
  }
};


// 
// REQUIREMENT TO MATCHER MAP
// Links each breadth requirement display name to its BREADTH_MATCHERS key.
// Used by isBreadthFulfilled() and the server's isDone() check.
// 
...
// 
// NORMALIZE CODE — STANDARDIZES COURSE CODES FOR COMPARISON
// Trims whitespace, uppercases, and collapses multiple spaces.
// 
...
// 
// NORMALIZE REQUIREMENT CODE — LIKE normalizeCode BUT ADDS SUFFIX IF MISSING
// If a code ends in digits with no L/N suffix, defaults to L.
// Handles cases where the AI returns "CMPT 220" instead of "CMPT 220L".
// 
...
// 
// MATCHES BREADTH — CHECKS IF A SINGLE COURSE CODE SATISFIES A BREADTH AREA
// Used by isBreadthFulfilled to test each completed course against a matcher.
// 
...
// 
// IS BREADTH FULFILLED — CHECKS IF ANY COMPLETED COURSE SATISFIES A BREADTH REQ
// Called from server.js during the cross-reference step.
// 
...
// 
// PATHWAYS — COURSE AREA DATA FOR EACH MARIST PATHWAY
// Used for server-side reference. Client-side rendering uses pathway_areas
// extracted directly from the PDF by the AI (so it works for any pathway).
//
// TO ADD A NEW PATHWAY: add a new key here matching Marist's pathway name exactly.
// 
const PATHWAYS = {
  "Quantitative Studies": {
    ...
    areas: [
      ...
      {
        // Area 6 is "Non-Breadth" — only ONE course allowed from this group.
        // Courses here do NOT count toward the 36 distribution credit total.
        area: 6,
...
// 
// ALL PATHWAYS — FULL LIST OF MARIST PATHWAY NAMES
// Used for future pathway comparison feature.
// 
const ALL_PATHWAYS = [
  "Quantitative Studies",
  "Humanistic Studies",
...
// 
// EXPORTS — ALL FUNCTIONS AND DATA USED BY SERVER.JS
// 
module.exports = {
  REQUIREMENTS,
  isBreadthFulfilled,
